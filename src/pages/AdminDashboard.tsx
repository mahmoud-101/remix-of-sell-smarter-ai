...
import { useNavigate } from "react-router-dom"; // عشان نطرده لو مش أدمن
import { 
...
import { useToast } from "@/hooks/use-toast"; // للتنبيه

// ---------------------------------------------------------
// 1. قائمة الأدمن (ضع إيميلك الحقيقي هنا)
// ---------------------------------------------------------
const ADMIN_EMAILS = [
  "r13028918@gmail.com", // إيميلك اللي كان في المحادثة
  // ممكن تضيف إيميلات تانية هنا لو معاك شركاء
];
...
export default function AdminDashboard() {
  ...
  const [isAdmin, setIsAdmin] = useState(false); // حالة التحقق
  ...
  useEffect(() => {
    const checkAdmin = async () => {
      ...
      if (!user || !ADMIN_EMAILS.includes(user.email || "")) {
        // لو مش موجود أو مش في القائمة، اطرده
        toast({
          variant: "destructive",
          title: "Access Denied ⛔",
          description: isRTL ? "ليس لديك صلاحية لدخول هذه الصفحة" : "You are not authorized to view this page.",
        });
        navigate("/dashboard"); // ارجع للداشبورد العادية
      } else {
        setIsAdmin(true); // تمام، ده الأدمن
      }
    };
    checkAdmin();
  }, [navigate, isRTL, toast]);

  // باقي الكود (Fetch Data) مش هيشتغل إلا لو هو أدمن
  const { data: profiles, isLoading: profilesLoading } = useQuery({
    queryKey: ["admin-profiles"],
    queryFn: async () => {
      if (!isAdmin) return []; // حماية إضافية: لو مش أدمن متجيبش بيانات
    ...
    enabled: isAdmin, // الكويري مش هتشتغل أصلاً غير لما نتأكد إنه أدمن
  });
  ...
  // ... (باقي كود الحسابات والرسم البياني زي ما هو)
  ...
  const stats = [
    ...
    {
      label: isRTL ? "إجمالي التوليدات" : "Total Generations",
    ...
    {
      label: isRTL ? "العملاء المحتملين" : "Total Leads",
      value: totalLeads,
      icon: TrendingUp,
      change: "+15%",
      positive: true,
      ...
      <div className="space-y-8">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            {isRTL ? "إدارة المستخدمين والاشتراكات والإحصائيات" : "Manage users, subscriptions, and statistics"}
          </p>
        </div>
